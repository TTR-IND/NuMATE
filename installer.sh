...
SETTINGS_API="https://api.github.com/repos/$SETTINGS_OWNER/$SETTINGS_NAME/releases"
...
# ── Fetch the newest release of a GitHub repo ──────────────────────────────
# Sets FETCH_DIR to the extracted source tree on success. Returns non-zero
# if no published release could be downloaded — there is deliberately NO
# fallback to a git clone. The development branch is experimental; silently
# building it when a release fetch fails would install something the user
# never asked for and did not know they were getting.
#
# Deliberately uses /releases (the list) and takes the first entry, NOT
# /releases/latest. GitHub's "latest" endpoint EXCLUDES pre-releases, so a
# repo whose newest release is marked pre-release returns 404 there.
# The list endpoint is ordered newest-first and includes pre-releases.
#
# Uses the zipball URL from the API rather than a named asset, because the
# asset filename is chosen by whoever cut the release and cannot be
# predicted. The zipball is generated by GitHub for every release.
FETCH_DIR=""

fetch_latest_release() {
    _f_api="$1"; _f_name="$2"; _f_dest="$3"
    FETCH_DIR=""

    # api.github.com allows 60 unauthenticated requests per hour per IP.
    # One request per install is well inside that.
    _json="$(curl -fsSL --connect-timeout 20 --retry 3 --retry-delay 3 \
                  -H 'Accept: application/vnd.github+json' \
                  "$_f_api" 2>/dev/null || true)"

    if [ -z "$_json" ]; then
        warn "$_f_name: could not reach the GitHub releases API"
        return 1
    fi

    # First zipball_url in the document = newest release. grep rather than
    # a JSON parser so this does not depend on jq, which is not installed
    # by default on Devuan.
    _zip_url="$(printf '%s' "$_json" | grep -o '"zipball_url"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | sed 's/.*"\(https[^"]*\)"/\1/')"
    _tag="$(printf '%s' "$_json" | grep -o '"tag_name"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | sed 's/.*"\([^"]*\)"$/\1/')"

    if [ -z "$_zip_url" ]; then
        warn "$_f_name: no published release found"
        return 1
    fi

    info "$_f_name: newest release is ${_tag:-unknown}"

    if ! curl -fsSL --connect-timeout 20 --retry 3 --retry-delay 3 \
              "$_zip_url" -o "$_f_dest/src.zip" 2>/dev/null; then
        warn "$_f_name: download of the release archive failed"
        return 1
    fi

    if ! unzip -q "$_f_dest/src.zip" -d "$_f_dest/x" 2>/dev/null; then
        warn "$_f_name: release archive is corrupt or truncated"
        return 1
    fi

    # GitHub zipballs contain a single top-level directory whose name
    # embeds the commit sha, so it is matched rather than assumed.
    FETCH_DIR="$(find "$_f_dest/x" -mindepth 1 -maxdepth 1 -type d | head -1)"
    if [ -z "$FETCH_DIR" ] || [ ! -f "$FETCH_DIR/Makefile" ]; then
        warn "$_f_name: release archive has no Makefile at its root"
        FETCH_DIR=""
        return 1
    fi

    ok "$_f_name ${_tag:-} downloaded"
    return 0
}
...
if [ "$OPT_SKIP_SETTINGS" -eq 1 ]; then
    ...
    # curl and unzip are needed to fetch a release; git only for the
    # fallback. Installed together so the fallback cannot fail for want of
    # a tool.
    apt_install "download tools" curl unzip ca-certificates >/dev/null || true

    _gs_dir="$(mktemp -d)"
    if [ "$OPT_DRY_RUN" -eq 1 ]; then
        info "[dry-run] fetch newest $SETTINGS_NAME release && make && make install"
    elif fetch_latest_release "$SETTINGS_API" "$SETTINGS_NAME" "$_gs_dir"; then
        _src="$FETCH_DIR"
        ...
        _mk_log="$(mktemp)"
        _build_ok=0
        if [ -n "$_src" ] && ( cd "$_src" && make ) >"$_mk_log" 2>&1; then
        ...
        if [ "$_build_ok" -eq 1 ]; then
            if ( cd "$_src" && sudo make install prefix=/usr/local ) \
                   >"$_mk_log" 2>&1; then
        ...
        warn "Could not obtain a published $SETTINGS_NAME release."
        warn "Check your connection, then re-run just this stage with:"
        warn "    ./installer.sh --only-stage=6"
    fi
    rm -rf "$_gs_dir"
fi
}
